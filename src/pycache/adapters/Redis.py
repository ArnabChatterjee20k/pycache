...
class Redis(Adapter):
    ...
    def __init__(self, connection_uri, pool_size=5, tablename="kv-store"):
        self._pool = redis.ConnectionPool.from_url(connection_uri, max_connections=pool_size,decode_responses=True)
    ...
    def _get_client(self):
        return self._transaction_pipeline if self._transaction_pipeline is not None else self._client
    ...
    async def batch_get(self, keys, datatype: Datatype = None):
        if isinstance(keys, dict):
            ...
            raise ValueError("keys must be either a list of strings or a dict mapping keys to datatype names")
    ...
    async def _set_datatype(self, key_path, datatype, pipe=None):
        ...
        try:
            ...
            if datatype_name not in  (String.get_name(),Numeric.get_name()) and not len(value):
                return 0
            ...
            logger.error(f"Error setting datatype {datatype_name} for key {key_path}: {e}")
            raise
    ...
    async def batch_set(self, key_values: dict[str, Datatype]) -> int:
        ...
        if self._transaction_pipeline is not None:
            ...
            for key, datatype in key_values.items():
                try:
                    await self._set_datatype(self.get_key_path(key), datatype,self._transaction_pipeline)
                    count += 1
                except Exception as e:
            ...
            async with self._client.pipeline() as pipe:
                ...
                for key, datatype in key_values.items():
                    try:
                        ...
                        logger.warning(f"Failed to queue set operation for key {key}: {e}")
                        failed_keys.append(key)
                ...
                if failed_keys:
                    logger.warning(f"Failed to set {len(failed_keys)} keys: {failed_keys}")
