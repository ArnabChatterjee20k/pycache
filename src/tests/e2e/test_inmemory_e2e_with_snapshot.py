...
class TestInMemoryCacheWithSnapshot(TestInMemoryCache):
    ...
    def create_cache(self):
        ...
        return PyCache(
            InMemory("memory://", snapshot=True, snapshot_config=snapshot_config)
        )
    ...
    async def test_snapshot_debug(self):
        ...
        if self.cache._adapter._snapshot:
            ...
            print(
                f"Snapshot min_changes: {self.cache._adapter._snapshot._config.min_changes}"
            )
            print(
                f"Snapshot interval_hours: {self.cache._adapter._snapshot._config.interval_hours}"
            )
    ...
    async def test_snapshot_creation_on_changes(self):
        ...
        ), f"Expected at least 1 snapshot, got {len(snapshot_files)}"
    ...
    async def test_snapshot_persistence_across_sessions(self):
        ...
        new_cache = PyCache(
            InMemory("memory://", snapshot=True, snapshot_config=new_snapshot_config)
        )
    ...
    async def test_snapshot_rotation_and_cleanup(self):
        ...
        async with self.cache.session() as cache:
            ...
            ), f"Expected 3 snapshots, got {len(snapshot_files)}"
    ...
    async def test_snapshot_with_complex_datatypes(self):
        ...
        async with self.cache.session() as cache:
            ...
            await cache.set(
                "map_key", Map({"nested": {"key": "value"}, "list": [1, 2, 3]})
            )
    ...
    async def test_snapshot_recovery_after_failure(self):
        ...
        recovery_cache = PyCache(
            InMemory("memory://", snapshot=True, snapshot_config=recovery_config)
        )
    ...
    async def test_snapshot_with_values(self):
        ...
        async with self.cache.session() as cache:
            ...
            await cache.set("mixed_data", List(["true", "false", "string", 42]))
        ...
        with open(latest_snapshot, "rb") as f:
            ...
            assert data["bool_true"]["value"] == "true"
            assert data["bool_false"]["value"] == "false"
            # Extract values from Datatype objects for comparison
            mixed_data = data["mixed_data"]["value"]
            assert mixed_data[0] == "true"
            assert mixed_data[1] == "false"

        # Loading it again in the cache
        async with self.cache.session() as cache:
            assert await cache.get("bool_true") == "true"
            assert await cache.get("bool_false") == "false"
            mixed_data = await cache.get("mixed_data")
            ...
            async def concurrent_operation(worker_id):
                for i in range(10):
                    await cache.set(
                        ...
                        String(f"worker_{worker_id}_value_{i}"),
                    )
    ...
    async def test_snapshot_with_large_data(self):
        ...
        async with self.cache.session() as cache:
            ...
            # Extract value from Datatype object for comparison
            if hasattr(result_list[999], "value"):
                assert result_list[999].value == "item_999"
            else:
                assert result_list[999] == "item_999"
            if hasattr(result_dict["key_499"], "value"):
                assert result_dict["key_499"].value == "value_499"
            else:
